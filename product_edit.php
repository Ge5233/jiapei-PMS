            ...
            <div>
                ...
                <div class="flex gap-1.5">
                    <div x-data="pmsSearchSelect({
                        ...
                        <!-- 选中：显示标签 -->
                        <div x-show="selectedId" class="ss-tag cursor-pointer" @click="openDropdown()">
...
async function refreshSuppliers() {
    ...
    if (d.ok) {
        // 新组件
        if (window._ssRefresh_sup) window._ssRefresh_sup(d.suppliers.map(function(s){ return {id:s.id,label:s.name}; }));
        // 兼容旧
        else if (window._supRefresh) window._supRefresh(d.suppliers);
    }
}
