...
- 大型系统主材/配件 CSS Grid 对齐 + 表头 + 背景色区分主次
  - ⚠️ **待优化**：主材/配件视觉区分不够明显，当前靠背景色（slate-100）和字号区分，后续需重新设计方案
- 大型系统主材默认外采、配件默认外采
- 配件名称选择后刷新不丢失修复（_prodShow 查产品表）
- 大型系统序列化优化：无产品选中→当临时存
- 报价/自产 列表页加 CSRF token 修复删不掉问题
- 指导售价 readonly 框加 hidden 字段修复保存为 0 的问题
